...
resource "local_file" "cloud_init_db_user_data_file" {
  ...
  content = templatefile("${path.module}/cloud-inits/cloud-init.db_cloud_config.tftpl", {
    ssh_key_j   = var.ssh_public_key_j
    ansible_key = var.ssh_public_key_ansible
    hostname    = each.key
  })
...
resource "null_resource" "cloud_init_db_config_files" {
  for_each = local.database_vms
  ...
  provisioner "file" {
    source      = local_file.cloud_init_db_user_data_file[each.key].filename
    destination = "/var/lib/vz/snippets/db_user_data_${each.key}.yml"
  }

  provisioner "file" {
    source      = local_file.cloud_init_db_network_file[each.key].filename
    destination = "/var/lib/vz/snippets/db_network_${each.key}.yml"
  }
}